# Oxyry Python Obfuscator (vou usar esse ofuscador porque é o melhor que achei até agora :v )
#codigo fonte: https://github.com/weijarz
...
def obfuscate_file(src, dest, options):
    ...
    try:
        ...
        sys.exit(1)
    # If requested, pass the obfuscated source to teste_ba for another layer
    if getattr(options, 'chain_teste', False):
        if not dest:
            print('error: --chain-teste requires --output to be specified', file=sys.stderr)
            sys.exit(1)
        try:
            import ofuscadores.ofuscador_py2 as ofuscador_py2
        except Exception:
            print('error: teste_ba module not found or failed to import', file=sys.stderr)
            sys.exit(1)
        # write intermediate obfuscated code to a temp file and let teste_ba create the final output
        import tempfile
        try:
            with tempfile.NamedTemporaryFile('w', delete=False, suffix='.py', encoding='utf-8') as tf:
                tf.write(obsrc)
                tfname = tf.name
            ofuscador_py2.criar_script_ofuscado(tfname, dest)
        except Exception as e:
            print('error: teste_ba.criar_script_ofuscado failed: {}'.format(e), file=sys.stderr)
            sys.exit(1)
        finally:
            try:
                os.remove(tfname)
                ...
                pass
        # We've delegated writing to teste_ba, so return now
        return
...
def parse_args(args):
    ...
    parser.add_argument('--chain-teste', '-t', action='store_true',
                        help='Send obfuscated output to teste_ba for an additional obfuscation pass')
...
def main(args=None):
    ...
    options.chain_teste = args.chain_teste
