...
var global_debug_level = 1 

func debug(str interface{}, debug_level ...int) {
    // Verificamos se algum nível de debug foi passado na lista
    if len(debug_level) > 0 {
        // Comparamos o primeiro item da lista com a variável global
        if debug_level[0] >= global_debug_level {
            fmt.Printf("[?] ")
            fmt.Println(str)
        }
    } else {
        // Se não passar nível, apenas imprime como debug padrão
        fmt.Printf("[?] ")
        fmt.Println(str)
    }
}
